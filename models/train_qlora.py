"""
QLoRA Fine-tuning: Teaching Llama 3.2-3B to Predict Prices
============================================================
...
YOUR MODEL:
    huggingface.co/maulik78/pricer-[timestamp]-lite
    MAE: $58.74 (vs XGBoost $68.23 -- 13.9% improvement)
"""
...
def load_dataset_splits():
    """Load the prompt dataset built in step5_build_prompts.py"""
...
def build_quant_config(use_bf16: bool):
    ...
    NF4 (NormalFloat4): places quantization buckets at normal
    distribution quantiles -- better precision where weights cluster.
    Double quantization: also quantizes quantization constants, saves ~0.4 GB.
    """
...
def load_model_and_tokenizer(quant_config):
    """
    Load tokenizer + 4-bit quantized Llama.
    pad_token = eos_token: Llama has no padding token, reuse EOS.
    padding_side = 'right': required by SFTTrainer.
    """
...
def build_lora_config():
    ...
    task_type='CAUSAL_LM': autoregressive text generation (each token predicts next).
    bias='none': don't add LoRA to bias terms (tiny, not worth it).
    """
...
def build_training_config(use_bf16: bool):
    ...
    hub_strategy='every_save': push checkpoint every SAVE_STEPS steps.
    If Colab crashes, last checkpoint is safe on HuggingFace.
    dataset_text_field='prompt': which field contains training text.
    """
...
def show_trainable_params(model):
    """
    Show what fraction of parameters are being trained.
    Key insight: 0.6% of parameters trained, 99.4% frozen.
    The frozen base preserves language understanding.
    The tiny adapters add pricing domain knowledge.
    """
